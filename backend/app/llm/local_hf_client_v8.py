            ...
            if _looks_degenerate_output(text):
                ...
                if _looks_degenerate_output(retry_text):
                    ...
                    if is_qa_like:
                        try:
                            ...
                            simple_text = (tok.decode(simple_ids, skip_special_tokens=True) or "").strip()
                ...
                if _looks_degenerate_output(retry_text):
                    ...
                    if any(k in p for k in ("qa", "answer", "chat", "domain")) and "code" not in p:
                        log.error(
                            "Base local model produced repeated-token output even with adapter disabled and simplified fallback."
                        )
                        return (
                            "The local model loaded successfully on cuda:0, but its generation collapsed into a repeated-token loop. "
                            "For Q&A, try setting LOCAL_LLM_MODEL=Qwen/Qwen2.5-1.5B-Instruct or TinyLlama/TinyLlama-1.1B-Chat-v1.0, "
                            "then restart the backend. The current coder model is not producing stable plain Q&A answers in this environment."
                        )

                    raise LLMUnavailable(
                        "Local model generated repeated-token output after retry. Use a chat-tuned local model or retrain/remove the unstable adapter."
                    )
