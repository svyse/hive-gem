...
def main() -> int:
    ...
    target_pairs = max(10, previous_pairs_used or min_new)
    ...
    if n_new < min_new and not force_train:
        if previous_pairs_used > 0:
            log.info(
                "Fewer than %s new items (%s); retraining on rolling window of most recent %s pairs.",
                min_new,
                n_new,
                len(pairs),
            )
            ...
            log.info(
                "Fewer than %s new items (%s); no prior training window recorded, training on most recent %s pairs.",
                min_new,
                n_new,
                len(pairs),
            )
    elif force_train:
        log.info("FORCE_TRAIN enabled; training on most recent %s pairs.", len(pairs))
    ...
    log.info(
        "Training local LoRA adapter | model=%s | device=%s | pairs=%s/%s recent pairs (conv=%s, extra=%s, include_upload_examples=%s, new_items=%s, previous_pairs_used=%s)",
        model_id,
        device,
        len(pairs),
        len(all_pairs),
    ...
    log.info(
        "Usable training samples=%s | max_seq_len=%s | target_tokens min/avg/max=%.0f/%.1f/%.0f",
        int(stats["count"]),
        train_max_len,
        stats["min"],
        stats["avg"],
