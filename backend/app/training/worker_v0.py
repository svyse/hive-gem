...
def main() -> int:
    ...
    if n_new < min_new and os.getenv("FORCE_TRAIN", "").strip().lower() not in {"1", "true", "yes"}:
        log.info("No training needed: %s new items since state=(msg=%s, ex=%s) (< %s)", n_new, last_msg_id, last_ex_id, min_new)
        return 0
    ...
    pairs = conv_pairs + ex_pairs
    if len(pairs) > max_pairs:
        pairs = pairs[-max_pairs:]

    if len(pairs) < 10:
        log.info("Not enough training pairs to train (found=%s)", len(pairs))
        return 0
    ...
    log.info(
        "Training local LoRA adapter | model=%s | device=%s | pairs=%s (conv=%s, extra=%s, include_upload_examples=%s)",
        model_id,
        device,
        len(pairs),
        len(conv_pairs),
